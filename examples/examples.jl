...
"""
function simulate(; N=30, nᵢ=fill(12, N), p=6, q=2, β=[5,10,20,10,0,0,0], 
    X=hcat(fill(1,Ntot), randn(Ntot, p)), Z=X[:, 1:q], L=10, σ²=100)
...
Random.seed!(54)
N = 300; nᵢ = 120; p=28  
β=repeat([5,10,20,10,0,0,0], 4)
X = hcat(fill(1, N*nᵢ), randn(N*nᵢ, p))
L = 30; σ² = 100
Xgrp, Zgrp, ygrp = simulate(; X=X, L=L, σ²=σ², p=28, q=3)


## Initial parameters 
β₀ = lasso(X, vcat(ygrp...))
println("Lasso estimated fixed effect are $β₀")
println("(L, σ²) calculated from Lasso estimates is $(cov_start(Xgrp, ygrp, Zgrp, β₀))")
