...
bgr_nz10 %>%
  ...
  mutate(fp = (n_nz - tp)/990,
         outlier = fp > quantile(fp, .75) + IQR(fp) * 1.5) %>%
  ungroup() %>%
  filter(outlier) %>%
  distinct(penalty, cov_str, q, predictor_cor)
```
...
bgr_nz10 %>%
  ...
  mutate(fp = (n_nz - tp)/990,
         outlier = fp > quantile(fp, .75) + IQR(fp) * 1.5,
         plot = TRUE) %>% 
  summarise(mn = mean(fp), med = median(fp), max = max(fp), n = n())
```
...
fp_otu <- best_otu_results %>%
  ...
  scale_y_continuous(sec.axis =
                       sec_axis(~ .,
                                name = "Feature correlation structure",
                                breaks = NULL,
                                labels = NULL)
                     ) +
  theme(#strip.text.x = element_text(size = 10 ),
...
tp_otu <- best_otu_results %>%
  group_by(q, cov_str, correlation) %>%
  mutate(
    outlier = tp < quantile(tp, .25) - IQR(tp)
  ) %>%
  ungroup() %>%
  ggplot(aes(y = tp, x = cov_str, col = penalty)) +
  geom_boxplot(width = .6, outlier.shape = NA) +
  geom_point(data = function(x) filter(x, outlier), 
             pch = 21, #aes(alpha = plot),
...
beta_grp_otu <- best_otu_results %>%  
  ...
  scale_y_continuous(breaks = c(-2,-1,0),
                     sec.axis =
                      sec_axis(~ ., 
                               name = "Feature matrix correlation structure",
                               breaks = NULL, 
                               labels = NULL)
                     ) +
...
var_otu <- best_otu_results %>%
  ...
  scale_y_continuous(n.breaks = 3,
                     sec.axis =
                       sec_axis(~ ., name = "Feature correlation structure",
                        breaks = NULL,
                        labels = NULL)
                     ) +
  coord_cartesian(ylim = c(0,1)) +
