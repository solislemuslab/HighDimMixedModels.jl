...
fp <- bgr_nz5 %>% 
  ...
  scale_x_discrete(position ="top", name = "Fixed effect dimension") +
  scale_color_brewer(type = "qual", labels = c("SCAD", "LASSO"))
fp
ggsave(
    "../plots/fp_nz5.pdf",
    fp,
...
best_gwas_results %>%
  mutate(
    ...
    cov_str = factor(
      ...
      filter(cov_str == "diag", penalty == "scad") %>%
      select(data_id, lambda, tp, beta_1, beta_2, beta_3, starts_with("psi"))
...
#### Diagonal and symmetric variance components

Not implemented yet. If we want to check these, we must divide by standard deviations because right now, Psi is on the scale of the standardized covariates.
...
fp_otu <- best_otu_results %>%
  ggplot(aes(y = (n_nz - tp)/117, x = cov_str, col = penalty)) +
  geom_boxplot(width = .3, outlier.size = 0.1) +
  labs(y = "False positive rate", x = "Random effect covariance") +
  ...
  scale_y_continuous(labels = scales::label_percent(accuracy=1), 
                     ...
                     ,sec.axis =
                       sec_axis(~ .,
                                name = "Design matrix correlation structure",
                                breaks = NULL,
                                labels = NULL)
                     ) +
  theme(#strip.text.x = element_text(size = 10 ),
...
tp_otu <- best_otu_results %>%
  ggplot(aes(y = tp, x = cov_str, col = penalty)) +
  geom_boxplot(width = .3, outlier.size = 0.1) +
  labs(y = "True positives (out of 11)", x = "Random effect covariance") +
  facet_grid(correlation ~ q, space = "free", scales = "free_x",
             labeller = labeller(q = q.labs)) + #Get rid of space argument if you want box plot width to automatically adjust to space in facet
  scale_y_continuous(labels = scales::label_number(accuracy = 1),
                     breaks = c(5,8,11)
...
beta_grp_otu <- best_otu_results %>%
  ...
  geom_hline(data = df_true, aes(yintercept = y_value, linetype = "True Parameter"),
             linewidth = .4) +
  labs(y = "Estimated parameter", x = "Random effect covariance") +
  facet_grid( ~ q, space = "free", scales = "free", 
             labeller = labeller(q = q.labs)) + #Get rid of space argument if you want box plot width to automatically adjust to space in facet
  scale_y_continuous(breaks = c(-5,-1,5), limits = c(-5,5),
                     # sec.axis = 
                     #   sec_axis(~ ., name = "Design matrix correlation structure",
                     #    breaks = NULL,
                     #    labels = NULL)
                     ) +
 theme(#strip.text.x = element_text(size = 10 ),
...
var_diag_otu <- best_otu_results %>%
  ...
  geom_segment(data = df_true, aes(x = parameter-.2, xend = parameter+.2,
                                   y = y_value, yend = y_value,
                                   linetype = "True Parameter")) +
  facet_grid( ~ 1) +
